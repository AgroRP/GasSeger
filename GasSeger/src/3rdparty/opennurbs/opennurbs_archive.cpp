...
)
{
	...
	if (IsDotOrDotDotDir((wchar_t*)m_fd.cFileName) || 0 != (m_file_attributes_mask & m_fd.dwFileAttributes))
	{
		return NextFile();
	}
...
{
#if defined(ON_COMPILER_MSC)
	return (0 != m_h && 0 != m_fd.cFileName[0]) ? (wchar_t*)m_fd.cFileName : 0;
#else
